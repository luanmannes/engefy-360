    ...
    <div className="animate-fade-up">
      <div className="flex items-center justify-between mb-8">
        <div>
          ...
          <p className="text-text-muted text-base">Classificacao dos colaboradores por desempenho nas avaliacoes.</p>
        </div>
          ...
          <button
            ...
            className={`px-4 py-2 rounded-lg font-head text-xs font-semibold uppercase tracking-wider transition-all ${
              filter === f.id
                ...
                : 'bg-surface text-text-muted border border-border-subtle hover:text-text-primary hover:bg-white/5'
            }`}
          >
            {f.label}
          </button>
          ...
          <div className="flex items-end justify-center gap-4 mb-8">
            {ranked.slice(0, 3).length >= 2 && (
              <div className="text-center">
                ...
                <div className="w-16 h-16 rounded-full bg-surface-2 border-2 border-border-subtle flex items-center justify-center font-head text-lg font-bold text-text-primary mx-auto mb-2">
                  {getInitials(ranked[1].name)}
                </div>
                <div className="font-head text-sm font-bold text-text-primary">{ranked[1].name}</div>
                <div className="text-xs text-text-muted">{ranked[1].totalScore} pts</div>
                <div className="w-20 h-16 bg-surface border border-border-subtle rounded-t-lg mt-3 flex items-center justify-center font-head text-2xl font-extrabold text-text-muted">
                  2
                </div>
              </div>
            )}
            {ranked.length >= 1 && (
              <div className="text-center">
                ...
                <div className="font-head text-base font-bold text-text-primary">{ranked[0].name}</div>
            ...
            {ranked.length >= 3 && (
              <div className="text-center">
                ...
                <div className="font-head text-sm font-bold text-text-primary">{ranked[2].name}</div>
                <div className="text-xs text-text-muted">{ranked[2].totalScore} pts</div>
                <div className="w-20 h-12 bg-surface border border-border-subtle rounded-t-lg mt-3 flex items-center justify-center font-head text-2xl font-extrabold text-text-muted">
                  3
                </div>
              </div>
            )}
            ...
            {ranked.map((p, i) => (
              <div
                ...
                className={`flex items-center gap-4 p-4 rounded-xl border transition-all ${
                  ...
                  i === 1 ? 'bg-surface border-border-subtle' :
                  i === 2 ? 'bg-surface border-border-subtle' :
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  <div className="font-head text-sm font-bold text-text-primary">{p.name}</div>
                  <div className="text-xs text-text-muted">{p.quizCount} modulo{p.quizCount !== 1 ? 's' : ''} · {p.quizNames.join(', ')}</div>
                </div>
                <div className="text-right">
                  <div className="font-head text-sm font-bold text-text-primary">{p.totalScore} pts</div>
                  <div className="text-xs text-text-muted">{p.avgPct}% aproveit.</div>
                </div>
              </div>
            ))}
          </div>
